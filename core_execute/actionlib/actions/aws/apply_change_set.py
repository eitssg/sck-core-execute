...
class ApplyChangeSetActionParams(BaseModel):
    ...
    stack_name: str = Field(..., alias="StackName", description="The name of the stack to apply the change set")
...
class ApplyChangeSetAction(BaseAction):
    ...
    def _execute(self):
        ...
        if self.get_state("ChangeSetApplicationStarted") and self.get_state("StackId"):
            log.info("Change set application already in progress for {}", self.params.change_set_name)
        ...
        try:
            change_set_response = cfn_client.describe_change_set(
                StackName=self.params.stack_name, ChangeSetName=self.params.change_set_name
            )
            ...
            if error_code == "ChangeSetNotFoundException":
                log.error("Change set '{}' not found for stack '{}'", self.params.change_set_name, self.params.stack_name)
                self.set_failed(f"Change set '{self.params.change_set_name}' not found for stack '{self.params.stack_name}'")
            else:
                log.error("Error describing change set '{}': {} - {}", self.params.change_set_name, error_code, error_message)
                self.set_failed(f"Failed to describe change set '{self.params.change_set_name}': {error_message}")
            return

        except Exception as e:
            log.error("Unexpected error describing change set '{}': {}", self.params.change_set_name, e)
        ...
        try:
            log.info("Executing change set {} on stack {}", self.params.change_set_name, self.params.stack_name)

            cfn_client.execute_change_set(StackName=self.params.stack_name, ChangeSetName=self.params.change_set_name)
            ...
            log.error("Error executing change set '{}': {} - {}", self.params.change_set_name, error_code, error_message)
            ...
            log.error("Unexpected error executing change set '{}': {}", self.params.change_set_name, e)
            self.set_state("ApplicationResult", "FAILED")
    ...
    def _check(self):
        ...
        try:
            ...
            if stack_status in ["UPDATE_COMPLETE", "CREATE_COMPLETE"]:
                ...
                log.info("Change set {} applied successfully to stack {}", self.params.change_set_name, self.params.stack_name)

            elif stack_status in ["UPDATE_IN_PROGRESS", "CREATE_IN_PROGRESS", "UPDATE_COMPLETE_CLEANUP_IN_PROGRESS"]:
                # Still applying
    ...
    def _unexecute(self):
        ...
        try:
            ...
            if stack_status in ["UPDATE_IN_PROGRESS", "CREATE_IN_PROGRESS"]:
                ...
                # Already rolled back
                log.info("Stack {} is already in rollback complete state", self.params.stack_name)
                self.set_state("RollbackResult", "ALREADY_ROLLED_BACK")
                self.set_complete(f"Stack {self.params.stack_name} is already rolled back")

            else:
                log.warning("Stack {} is in status {} - rollback may not be applicable", self.params.stack_name, stack_status)
