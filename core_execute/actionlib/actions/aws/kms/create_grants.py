...
class CreateGrantsActionParams(BaseModel):
    ...
    def validate_operations(cls, v):
        ...
        valid_operations = {
            ...
            "GenerateDataKeyPair", "GenerateDataKeyPairWithoutPlaintext", "GetPublicKey",
            "Sign", "Verify"
        }
...
class CreateGrantsAction(BaseAction):
    ...
    def _execute(self):
        ...
        try:
            ...
            for principal in self.grantee_principals:
                ...
                try:
                    ...
                    grant_info = {
                        ...
                        "KeyId": self.kms_key_id,
                        "CreatedAt": util.get_timestamp_str(),  # Add timestamp
                    }
                    ...
                    failure_info = {
                        ...
                        "FailedAt": util.get_timestamp_str(),
                    }
                    ...
                    if ignore_failures:
                        ...
                        log.error("Failed to create grant for principal '{}' - {}", principal, e)
            ...
            log.info(
                ...
                successful_grants, failed_grants, ignore_failures
            )
            self.set_complete()
    ...
    def _store_grant_state(self, created_grants: list, failed_principals: list, successful_count: int, failed_count: int):
        """
        ...
        self.set_output("ExecutionTimestamp", util.get_timestamp_str())
    ...
    def _check(self):
        ...
        try:
            ...
            self.set_output("LastChecked", util.get_timestamp_str())
            
            if inactive_grants:
                log.warning("Found {} inactive grants out of {} total", 
                           len(inactive_grants), len(created_grants))
    ...
    def _unexecute(self):
        ...
        try:
            ...
            self.set_output("RetirementTimestamp", util.get_timestamp_str())
            
            if failed_retirements:
                log.warning("Failed to retire {} grants out of {} total", 
                           len(failed_retirements), len(created_grants))
            
            log.info("Grant retirement completed: {} retired, {} failed", 
                    len(retired_grants), len(failed_retirements))
    ...
    def _resolve(self):
        ...
        try:
            ...
            self.operations = self.renderer.render_object(
                self.operations, self.context
            )
