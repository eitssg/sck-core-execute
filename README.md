...
Since the file/script is now in the file ``core_execute/actionlib/actions/aws/kms/create_grants.py`` we will derrive the name of the action for the ActionResource API by:
...
When this class is instantated by the engine, it will pass the ``ActionResource`` and ``DeploymentDetails`` to the ``__init__()`` function for use in the action script.
