...
@pytest.fixture
def get_mock_shared_snapshot():
    # Create snapshot mocks that return actual dictionaries
...
@pytest.fixture
def mock_source_ec2_resource():
    """Mock source EC2 resource"""
    mock_source_ec2_resource = MagicMock()

    # Mock source snapshot
...
@pytest.fixture
def mock_target_ec2_resource(get_mock_shared_snapshot, get_mock_copied_snapshot):
    mock_target_ec2_resource = MagicMock()

    # Configure target EC2 resource snapshots
    def mock_target_snapshot_side_effect(snapshot_id):
        if snapshot_id == "snap-source123":
            return get_mock_shared_snapshot
        else:
            return get_mock_copied_snapshot

    mock_target_ec2_resource.register_image.return_value = {"ImageId": "ami-target123"}

    # Mock target account snapshots
...
def test_duplicate_image_to_account(
    ...
    mock_target_ec2_resource,  # Fixed: use correct fixture name
    mock_source_ec2_client,
    mock_source_ec2_resource,  # Now defined above
):
    try:
