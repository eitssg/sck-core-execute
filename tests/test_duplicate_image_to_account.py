...
action_name = "duplicate-image-to-account-test"
...
def deploy_spec() -> DeploySpec:
    ...
    spec = DuplicateImageToAccountActionSpec.model_validate(spec_params)

    action_resource = DuplicateImageToAccountActionResource(name=action_name, spec=spec)

    return DeploySpec(actions=[action_resource])


def setup_source_ec2_client(mock_source_ec2_client):
...
def setup_target_ec2_client(mock_target_ec2_client):
...
def test_duplicate_image_to_account(
    ...
    try:
        ...
        # Get's the boto3 session Mock and cache it
        session = get_session()

        # Create and cache temporary credentials for source and target accounts as indexed by role ARN
        source_creds = get_role_credentials(RoleArn=util.get_provisioning_role_arn("154798051514"))
        target_creds_1 = get_role_credentials(RoleArn=util.get_provisioning_role_arn("123456789012"))
        target_creds_2 = get_role_credentials(RoleArn=util.get_provisioning_role_arn("123456789013"))

        mock_source_ec2_client = session.client("ec2", client_type="source", aws_account_id="154798051514", **source_creds)
        mock_source_ec2_client = setup_source_ec2_client(mock_source_ec2_client)

        mock_target_ec2_client1 = session.client("ec2", client_type="target1", account_id="123456789012", **target_creds_1)
        mock_target_ec2_client1 = setup_target_ec2_client(mock_target_ec2_client1)

        mock_target_ec2_client2 = session.client("ec2", client_type="target2", aws_account_id="123456789013", **target_creds_2)
        mock_target_ec2_client2 = setup_target_ec2_client(mock_target_ec2_client2)
        ...
        mock_source_ec2_resource = session.resource("ec2", **source_creds)
        mock_source_ec2_resource.Snapshot.return_value = mock_source_snapshot

        # The instance of the MagicMock is identified by the credentials signature

        def mock_target_snapshot_side_effect(snapshot_id) -> MagicMock:
        ...
        # The instance of the MagicMock is identified by the credentials signature
        mock_target_ec2_resource1 = session.resource("ec2", **target_creds_1)
        mock_target_ec2_resource1.Snapshot.side_effect = mock_target_snapshot_side_effect

        # The instance of the MagicMock is identified by the credentials signature
        mock_target_ec2_resource2 = session.resource("ec2", **target_creds_2)
        mock_target_ec2_resource2.Snapshot.side_effect = mock_target_snapshot_side_effect
        ...
        assert f"var/{action_name}/DuplicationStarted" in state
        assert f"var/{action_name}/SourceImageId" in state
        assert f"var/{action_name}/SuccessfulAccounts" in state
        assert f"var/{action_name}/CreatedImages" in state

        # Verify the actual values
        source_image_id = state[f"var/{action_name}/SourceImageId"]
        successful_accounts = state[f"var/{action_name}/SuccessfulAccounts"]
        created_images = state[f"var/{action_name}/CreatedImages"]
